      ...
      .then((data) => {
        if (data && data.features) {
          const points = data.features.map((f) => {
            // 1. Read the REAL size of the threat from the backend data
            const pixelCount = f.properties.count || 0;
            
            // 2. Determine the real threat level based on the affected area size.
            // If the cluster has more than 50 high-risk pixels, it's an Extreme emergency.
            const realSeverity = pixelCount > 50 ? 'extreme' : 'warning';

            return {
              lng: f.geometry.coordinates[0],
              lat: f.geometry.coordinates[1],
              severity: realSeverity,
              affectedPixels: pixelCount // We save this to show in the popup!
            };
          });
          setWarnings(points);
        }
            ...
            <div className="font-sans -m-1">
              ...
              <div className="p-4 bg-white">
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 mb-3">
                  ...
                  {/* --- NEW: Dynamic real threat data injected here --- */}
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-gray-500">Affected Pixels:</span>
                    <span className="text-gray-900 font-bold">{point.affectedPixels} units</span>
                  </div>
