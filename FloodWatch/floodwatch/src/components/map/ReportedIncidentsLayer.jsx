...
const createHazardIcon = () =>
  L.divIcon({
    html: `
      <div class="flex items-center justify-center w-8 h-8 rounded-full bg-orange-500 border-2 border-white shadow-[0_0_10px_rgba(249,115,22,0.8)] hover:scale-110 transition-transform animate-bounce">
        ${hazardSvg}
      </div>
    `,
    className: '',
...
export default function ReportedIncidentsLayer() {
  const { reportedIncidents } = useAppState();

  // IMPORTANT:
  // Do not listen for map clicks here.
  // FloodMap.jsx already handles reportingMode clicks.
  // Having two useMapEvents click handlers can cause confusing behavior.
